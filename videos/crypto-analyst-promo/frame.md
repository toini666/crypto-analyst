...
description: >
  ...
  accent, the three-step gray text ladder, IBM Plex Sans (display/numerals/chrome) + IBM Plex Sans (body),
...
colors:
  ...
  text-muted: "#9AA3B2"
  text-light: "#5C6470"
  accent-light: "rgba(217, 161, 59, 0.08)"
  accent-medium: "rgba(217, 161, 59, 0.15)"
  border: "rgba(217, 161, 59, 0.2)"
  card-bg: "rgba(217, 161, 59, 0.04)"
  positive: "#57C98B"
  negative: "#E2574B"
...
typography:
  # — reading ramp (IBM Plex Sans body + chrome) —
  body:    { fontFamily: "IBM Plex Sans", cqw: 0.85, weight: 400, lineHeight: 1.6, color: "text-muted" }
  h4-eyebrow:{ fontFamily: "IBM Plex Sans", cqw: 0.8, weight: 600, tracking: "0.08em", upper: true, color: "primary" }
  tag:     { fontFamily: "IBM Plex Sans", px: 12, weight: 500, color: "primary" }
  # — numerical ramp (IBM Plex Mono, tabular-nums, amber) — the brand's number face: "le chiffre est sacré" —
  counter: { fontFamily: "IBM Plex Mono", px: 13, weight: 500, tracking: "0.05em", color: "text-muted" }
  stat-num:{ fontFamily: "IBM Plex Mono", cqw: 1.9, weight: 600, lineHeight: 1.0, color: "primary" }
  metric-value:{ fontFamily: "IBM Plex Mono", cqw: 3.0, weight: 600, lineHeight: 1.0, color: "primary" }
  # — display ramp (IBM Plex Sans headings, −0.02em near-white) —
  h3:      { fontFamily: "IBM Plex Sans", cqw: 1.25, weight: 500, lineHeight: 1.3, tracking: "-0.02em", color: "text" }
  h2:      { fontFamily: "IBM Plex Sans", cqw: 2.6, weight: 600, lineHeight: 1.1, tracking: "-0.02em", color: "text" }
  h1:      { fontFamily: "IBM Plex Sans", cqw: 4.2, weight: 700, lineHeight: 1.08, tracking: "-0.02em", color: "text" }
  # — editorial / verdict ramp (Newsreader serif italic) — used sparingly, as the app reserves it for verdicts —
  blockquote:{ fontFamily: "Newsreader", cqw: 2.4, weight: 500, lineHeight: 1.35, color: "text", italic: true }
  quote-mark:{ fontFamily: "Newsreader", cqw: 8.0, weight: 600, lineHeight: 0.5, color: "primary", opacity: 0.15 }
...
- **Fonts** — tri-face brand system: **IBM Plex Sans** (display headings + all chrome), **IBM Plex Mono** (every numeral / metric / counter, always `tabular-nums`), **Newsreader** serif *italic* (editorial verdict / pull-quote moments ONLY — used sparingly, exactly as the app reserves serif for verdicts). Ignore any preset font name lingering in prose, and read the "no italic / near-black headline" preset rules through this: our ground is near-black **#0B0D11**, headlines are near-**white** `text`, and the one italic-serif moment is the verdict/quote.
...
The voice is two faces in fixed roles: **IBM Plex Sans** (display, every numeral, all chrome —
eyebrows uppercase 0.08em) and **IBM Plex Sans** (body, muted gray, line 1.6). Headlines are near-black;
...
- **IBM Plex Sans** (display/numerals/chrome) + **IBM Plex Sans** (body) — near-black headlines, cobalt numerals.
...
Two ramps. The **reading ramp** (IBM Plex Sans body 0.85cqw muted; IBM Plex Sans eyebrow uppercase 0.08em
cobalt) carries copy + chrome; the **display/numerical ramp** (IBM Plex Sans `h3` 1.25cqw → `h1`
...
- **Headlines near-black, −0.02em**; **eyebrows cobalt, uppercase, 0.08em**; **numerals cobalt 600–700**; **body IBM Plex Sans 400 muted, line 1.6**. No italic, no uppercase body, no cobalt headline.
...
**Composes** accent-line, meta, h1, body sub. **Focal** a 2-line IBM Plex Sans `h1` near-black, left,
...
**Focal** a row of tinted cards — cobalt `metric-value` + IBM Plex Sans label + muted desc + optional
...
blockquote, cite. **Focal** a IBM Plex Sans `blockquote` near-black under a 15%-opacity cobalt
...
IBM Plex Sans body column beside a cobalt-8% highlight block (4px cobalt left rule) carrying an inline pull
...
a IBM Plex Sans `h1` near-black, centered, with the one solid cobalt `cta-button` pill below. **Accent**
...
- Use **tinted cards** (4% fill, 20% border, 10–14px radius, no shadow); body IBM Plex Sans 400 muted, line 1.6.
...
- **Type** — IBM Plex Sans headings −0.02em near-black, cobalt eyebrows 0.08em + numerals; IBM Plex Sans body muted line 1.6; ≥1.4cqw floor.
...
- **IBM Plex Sans + IBM Plex Sans via Google Fonts.** CJK pairing (Noto Sans SC 700 display / Noto Serif SC 400 body) carries over; the eyebrow's uppercase+tracking signal weakens in CJK — pair it with the accent-line.
