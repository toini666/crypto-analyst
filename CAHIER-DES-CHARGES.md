...
2. **Méthodologie transparente et versionnée** : la grille de scoring (piliers, poids, seuils, vetos) est une configuration explicite ; chaque rapport référence la version de méthodologie utilisée, ce qui rend les analyses comparables dans le temps.
...
- L'application tourne **en local sur sa machine** (app web Next.js + base Supabase locale). Il l'utilise typiquement le soir, sur un seul écran, pour des sessions de lecture longue.
...
| **Marché & valorisation** | 15 % | Liquidité Volume/MC (35 %, quant) · Valorisation vs revenus P/S (20 %, quant) · Valorisation vs comparables (45 %, qual) |
...
- **Majeur** (ex. top 10 holders > 40 % de l'offre, MC/FDV < 0,25, Volume/MC < 5 %, privilèges propriétaire dangereux) ;
- **Mineur** (ex. Volume/MC < 10 %, GitHub inactif depuis 90 jours, prix > 90 % sous l'ATH).
...
### 3.4 Vetos (plafonds de score)

C'est la spécificité méthodologique de l'outil : une moyenne pondérée peut masquer un défaut fatal, donc certains red flags **plafonnent** le score global quel que soit le reste :

- red flag **critique** → score plafonné à **20** ;
- red flag **majeur** → score plafonné à **55**.

Quand un veto s'applique, l'application affiche : la raison du veto, le plafond appliqué, et le **score avant veto** (ex. « Veto appliqué : … — score plafonné à 55 (score avant veto : 68) »). C'est une information capitale qui doit sauter aux yeux dans toute proposition de design.
...
Chaîne sémantique (ex. « v1.1.0 ») affichée sur chaque analyse (liste et rapport). Deux analyses ne sont strictement comparables que si leur version de méthodologie est identique — d'où son affichage systématique.
...
- Toute l'UI est **temps réel** (Supabase Realtime) : la liste des analyses et la page de progression se mettent à jour sans rechargement.
...
- si un **veto** a été appliqué : encart bien visible « Veto appliqué : <raison> — score plafonné à <N> (score avant veto : <M>) » ;
...
**B. Le rapport complet**, rendu depuis le Markdown (tableaux inclus). C'est un document long (plusieurs milliers de mots) destiné à la **lecture approfondie** ; la typographie de lecture longue est le cœur du produit. Sa structure exacte :
...
2. **Executive Summary** : description du projet en 2 lignes + narratif (catégorie : DeFi, L1, meme…) ; verdict + score global ; rappel des vetos éventuels ; **tableau des 6 piliers** (poids, score, couverture) ; **3 forces / 3 faiblesses** ; **thèse d'investissement** en un paragraphe.
...
4. **Analyse détaillée par pilier** : pour chacun des 6 piliers (titre + score + poids), chaque composant avec son score :
   - composant *quantitatif* → « Constat (déterministe) » : justification chiffrée produite par le code (ex. « Volume/MC = 4,2 % : sous le seuil de liquidité ») ;
...
2. **Lecture rapide (< 3 min)** : ouvrir une analyse terminée → verdict, score, vetos, red flags, scores de piliers, forces/faiblesses → décision « ça mérite ou non que je creuse ».
...
3. **La sévérité est visible** : red flags et vetos ne sont jamais esthétiquement adoucis ; un veto qui plafonne le score doit se voir instantanément.
...
- Données servies par Supabase local ; mises à jour temps réel par subscriptions (la liste comme le détail réagissent à des événements INSERT/UPDATE).
- Le rapport est stocké en **Markdown** et rendu côté client (support GFM : tableaux, listes de tâches) — le design doit donc styler du contenu Markdown généré, pas des composants au cas par cas.
