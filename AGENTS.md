...
- `pnpm export-seed` : (ré)écrit la **graine d'analyses versionnée** `seed/analyses.ndjson` (une analyse terminée par ligne, `select *` de la table `analyses` — colonnes JSON incluses) depuis la base. **Le portefeuille n'est jamais exporté** (données perso). À relancer après avoir produit/re-scoré des analyses qu'on veut publier dans le dépôt.
- `pnpm seed` : réimporte cette graine dans la base locale — idempotent (`insert or ignore` par id : n'écrase aucune analyse existante, ne touche pas au portefeuille). Utile pour récupérer les analyses déjà réalisées sans repartir de zéro. **Auto-appliqué au premier lancement sur une base fraîche** (fichier `data/app.db` absent, ex. clone du dépôt) via `src/lib/db/index.ts` — un clone démarre donc directement avec les analyses existantes. La graine porte `id/created_at/…` verbatim (les liens et noms de fichiers `reports/` restent stables) ; les `analysis_events` (log de progression éphémère) ne sont pas seedés.
